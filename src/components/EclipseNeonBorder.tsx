...
export function EclipseNeonBorder({ isPlaying }: EclipseNeonBorderProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Sizing canvas to match client dimensions
      const width = canvas.width;
      const height = canvas.height;
      const centerX = width / 2;
      const centerY = height / 2;

      // The sphere radius is ~44% of canvas width (leaving outer ~12% for the coronal flares)
      const baseRadius = (width / 2) * 0.81;

      // Sample Audio Engine frequency data
      let rawIntensity = 0;
      let rawBass = 0;
      let rawTreble = 0;
      ...
      try {
        ...
        if (freqBins && freqBins.length > 0 && isPlaying) {
          ...
          // Subtle breathing simulation if analyser is warming up
      ...
      // Smooth the transitions for organic, liquid inertia
      ...
      // Color rotation moves faster when music is intense
      colorRotation += 0.01 + smoothIntensity * 0.04;

      // Clear previous frame
      ctx.clearRect(0, 0, width, height);

      // Dynamic Palette: Shifts between Electric Cyan, Neon Magenta, Celestial Violet, and Solar Gold
      ...
      // 1. OUTMOST DIFFUSE CORONA AURA (Deep atmospheric glow)
      ...
      // 2. SOLAR ECLIPSE CORONA STREAMERS (Rayos de plasma solares interactivos)
      const rayCount = 120;
      ...
      for (let i = 0; i < rayCount; i++) {
        ...
        // Sample frequency for this ray direction
        let freqVal = 0;
        if (freqBins && freqBins.length > 0) {
        ...
        const rayLength = baseRayLen + reactiveRayLen + noiseWobble;
        ...
        const rayAlpha = Math.min(1, 0.25 + freqVal * 0.65 + smoothIntensity * 0.35);
      ...
      // 3. INNER NEON CORONA RIM (Radiant Edge Ribbon)
      ctx.save();
      ctx.beginPath();
      ...
      // Second sharp neon ring overlay
      ctx.beginPath();
      ...
      // 4. THE DIAMOND RING EFFECT (Destello del Anillo de Diamante de Eclipse)
      // Rotates along the top-right perimeter and flares with music bursts
